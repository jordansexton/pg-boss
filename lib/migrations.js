...
function getMigrations(schema) {
    return [{
        ...
        uninstall: ['ALTER TABLE ' + schema + '.job ADD expiredOn timestamp without time zone', 'DROP INDEX ' + schema + '.job_singletonKey', 'DROP INDEX ' + schema + '.job_singletonOn', 'DROP INDEX ' + schema + '.job_singletonKeyOn', 'ALTER TABLE ' + schema + '.job DROP COLUMN singletonKey', 'ALTER TABLE ' + schema + '.job ALTER COLUMN state SET DATA TYPE text', 'DROP TYPE ' + schema + '.job_state',
